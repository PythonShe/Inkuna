...
fn a_declared_non_image_is_never_persisted_as_a_cover() {
    assert_eq!(
        image_extension("application/xhtml+xml", "cover.xhtml"),
        None
    );
...
fn declared_images_keep_the_href_fallback() {
    assert_eq!(
        image_extension("image/jpeg", "表紙.jpg").as_deref(),
        Some("jpg")
    );
    assert_eq!(
        image_extension("image/avif", "表紙.AVIF").as_deref(),
        Some("avif")
    );
    assert_eq!(
        image_extension("image/heic", "表紙").as_deref(),
        Some("img")
    );
...
fn a_missing_media_type_falls_back_to_a_known_image_suffix() {
    assert_eq!(
        image_extension("", "images/表紙.jpeg").as_deref(),
        Some("jpeg")
    );
